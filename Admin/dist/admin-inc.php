<?php
    ...
    if(isset($_GET['action']) && isset($_GET['action']) && isset($_GET['action']) == 'changestatus'){
        ...
        $sql = "UPDATE booking set status_book='$status_id' where booking_id='$booking_id'"; 
        $query = mysqli_query($con, $sql); //คิวรี่ข้อมูลเข้า database
        header("Location:normalSeat.php?update=Success"); //link ไปหน้าที่กำหนดไว้
        exit();
        ...
        header("Location:normalSeat.php?update=Success");
        exit();
